...
const BoatListing = () => {
  ...
  const { data: boatData, isLoading, error } = useQuery({
    queryKey: ['boat', id],
  ...
  const { data: similarBoatsData } = useQuery({
    ...
    queryFn: () => boatService.searchBoats({
      type: boatData?.type,
    }),
    select: (data) => data.filter(boat => boat.id !== boatData?.id).slice(0, 4),
        ...
        <main className="flex-grow">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <Alert variant="destructive">
              ...
              <AlertDescription>
                Tekne detayları yüklenirken bir hata oluştu. Lütfen daha sonra tekrar deneyin.
              </AlertDescription>
            </Alert>
          </div>
        </main>
          ...
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              <BoatInfo boat={{
                ...
                captainOption: boatData.captainIncluded ? 'Captain Included' : 'Optional Captain',
                ...
                <Card className="p-6">
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-xl font-semibold">{boatData.name} Sahibi</h3>
                      <div className="mt-2 space-y-2 text-gray-600">
                        <p>Yanıt oranı: 95%</p>
                        <p>Yanıt süresi: ~1 saat</p>
                      </div>
                      <p className="mt-4 text-gray-700">
                        Profesyonel kaptan ve tekne sahibi. Türk Denizcilik Kurumu sertifikalı.
                      </p>
                    </div>
                  </div>
                </Card>
              ...
              <div className="mt-16">
                ...
                {similarBoatsData && similarBoatsData.length > 0 ? (
                  ...
                  <div className="text-center py-8 bg-gray-50 rounded-lg">
                    <p className="text-gray-600">Bu kategoride başka tekne bulunamadı.</p>
                  </div>
                )}
              </div>
            </div>

...
const mockVessels = [
  {
    ...
    thumbnailUrl: "https://images.unsplash.com/photo-1527679124583-9208be990bb5?q=80&w=1000"
  },
  {
    id: "2",
    name: "Sea Paradise",
    ...
    thumbnailUrl: "https://images.unsplash.com/photo-1599054802207-91d346adc120?q=80&w=1000"
  },
  {
    id: "3",
    name: "Swift Wave",
    ...
    thumbnailUrl: "https://images.unsplash.com/photo-1605281317010-fe5ffe798166?q=80&w=1000"
  }
];
...
interface VesselsListProps {
  ...
  isEmpty?: boolean; // For demonstration purposes
}
...
const VesselsList: React.FC<VesselsListProps> = ({
  ...
  useEffect(() => {
    ...
    // Apply type filter
    if (typeFilter !== "all") {
      results = results.filter(vessel => vessel.type === typeFilter);
    }
    ...
    if (capacityFilter !== "all") {
      switch (capacityFilter) {
        case "1-6":
          results = results.filter(vessel => vessel.capacity >= 1 && vessel.capacity <= 6);
          break;
        case "7-12":
          results = results.filter(vessel => vessel.capacity >= 7 && vessel.capacity <= 12);
          break;
        case "13-20":
          results = results.filter(vessel => vessel.capacity >= 13 && vessel.capacity <= 20);
          break;
        case "21+":
        ...
        <>
          ...
          {filteredVessels.length === 0 ? (
            <div className="bg-white p-6 rounded-lg text-center">
              <p className="text-gray-500">Arama kriterlerinize uygun taşıt bulunamadı.</p>
            </div>
          ) : (

...
const BoatListingHeader: React.FC<BoatListingHeaderProps> = ({
  ...
  return (
    <div className="flex items-center justify-between mb-6 bg-white/80 backdrop-blur-sm rounded-xl p-6 border border-gray-200/50 shadow-sm">
      ...
      <div className="flex items-center space-x-4">
        <div className="flex items-center space-x-2">
          <span className="text-sm text-gray-600">{t.pages.boats.hourly}</span>
          <Switch
            checked={!isHourlyMode} // knob right => Günlük
            onCheckedChange={(checked) => setIsHourlyMode(!checked)}
            className="data-[state=checked]:bg-blue-600"
          />
          <span className="text-sm text-gray-600">{t.pages.boats.daily}</span>
        </div>
        ...
        <Select value={sortBy} onValueChange={setSortBy}>
          ...
          <SelectContent>
            <SelectItem value="popular">
              {t.pages.boats.sorting.popular}
            </SelectItem>
            <SelectItem value="price-low">
              {t.pages.boats.sorting.priceLow}
            </SelectItem>
            <SelectItem value="price-high">
              {t.pages.boats.sorting.priceHigh}
            </SelectItem>
            <SelectItem value="capacity">
              {t.pages.boats.sorting.capacity}
            </SelectItem>
            <SelectItem value="rating">
              {t.pages.boats.sorting.rating}
            </SelectItem>
          </SelectContent>
        </Select>
